(function (global, factory) {
  ...
  class dbIndexed {
     ...
     * @constructor
     * @public
     * @param {string} dbName Indesed DB name
     */
    constructor(dbName) {
            ...
            where: function (key) {
              ...
              if (typeof key === 'string') {
                return {
                    ...
                    return new Promise((resolve, reject) => {
                      ...
                      request.onsuccess = () => {
                        ...
                        objectStore.openCursor().onsuccess = function (e) {
                          var cursor = e.target.result;
                          if (cursor) {
                            if (cursor.value[key].includes(value)) list.push(cursor.value);
                            cursor.continue();
                          }
                          else {
                            resolve(list);
                  ...
                  notContains: async function (value) {
                    return new Promise((resolve, reject) => {
                      var request = indexedDB.open(this.dbName, this.version);
                      request.onerror = console.log;
                      request.onsuccess = () => {
